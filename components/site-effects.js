// Wrapped in an IIFE so const declarations are locally scoped.
// This means even if the script tag is somehow evaluated twice,
// it will never throw "ERROR_PAGE already declared".
(function () {
  ...
  function isSiteNavigationClick(el) {
    return Boolean(
      el.closest(
        '#site-header, .site-header, .navbar, .desktop-nav, .nav-links, .nav-links a, .nav-actions, .nav-actions a, .hamburger-btn, .mobile-drawer, .mobile-drawer a, .mobile-drawer__btn, .mobile-drawer__back, .btn-nav, .auth-back-btn, .auth-page a, .footer-col--links a, .footer-logo'
      )
    );
  }

  let footerSocialRedirectBound = false;
  let buttonRedirectBound = false;

  function initFooterSocialRedirect() {
    if (footerSocialRedirectBound) return;
    footerSocialRedirectBound = true;
  ...
  function initButtonRedirect() {
    if (buttonRedirectBound) return;
    buttonRedirectBound = true;
    ...
    document.addEventListener('click', (e) => {
      ...
      const target = clicked.closest(
        'button:not(.hamburger-btn):not(.mobile-drawer__close):not(.mobile-drawer__back):not(.toggle-password):not(.faq-question):not(.auth-back-btn), a.btn:not(.btn-nav):not(.mobile-drawer__btn):not(.auth-back-btn), a.primary-btn, a.secondary-btn, a.cta-btn, .auth-submit, button.auth-social-btn, .price-card button'
      );
